package com.lynda.core.mybatis;
...
import com.lynda.core.mybatis.page.MysqlDialect;
...
public class BaseMybatisDao<T> extends SqlSessionDaoSupport {
	/**
	 * 发现是mybatis取消了自动注入SqlSessionFactory 和 SqlSessionTemplate
	 *	通过在BaseMybatisDao中注入该属性，其他dao继承该AbstractBaseDao
	 */
	@Resource  
	...
	public Pagination findByPageBySqlId(String sqlId,
		...
		try {
			//不知道为什么这个地方得到的connection一直是closed=true，所以查询会报错，应为连接池是关闭的
			//所以这里的分页不用了，用mybatis的拦截器分页（package=plugin）
