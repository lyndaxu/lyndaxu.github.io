...
import com.lynda.util.VerifyCodeUtils;
import com.lynda.util.vcode.Captcha;
import com.lynda.util.vcode.GifCaptcha;
import com.lynda.util.vcode.SpecCaptcha;
...
public class CommonController extends BaseController {
	...
	public ModelAndView _500(HttpServletRequest request){
		...
		if(null != stack && stack.length != 0 ){
			...
			logger.fmtError("line:%s,clazz:%s,fileName:%s,methodName:%s()",
	...
	public void getVCode(HttpServletResponse response,HttpServletRequest request){
		try {
			...
			logger.fmtError(e, "获取验证码异常：%s",e.getMessage());
		}
	}
