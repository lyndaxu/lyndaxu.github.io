...
function saveD() {
	...
	var fields = dname + ',mbfw,' + dtype + ',mbfw,' + dbz + ',mbfw,' + isQian + ',mbfw,' + ddefault;
...
function editField(value, msgIndex) {
	var efieldarray = value.split(',mbfw,');
...
function appendC(value) {
	var fieldarray = value.split(',mbfw,');
...
function isSame(value) {
	for (var i = 0; i < arField.length; i++) {
		var array0 = arField[i].split(',mbfw,')[0];
		if (array0 == value) {
			return false;
		}
	}
