...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
        ...
        //是否初始化默认执行noFocus:true不执行 edit by folger.qin 2012-8-22
		if(!config.noFocus){
...
lhgdialog.tips = function( content, time, icon, callback )
{
    ...
    /**
     * 防止重复提交提示
     * @param   {Number}    显示时间 (默认20秒)
     * @param	{String}	提示内容
     * @param	{String}	提示图标 (注意要加扩展名)
     * @param   {Boolean}  打开时是否锁屏
     * @param   {Function}  提示关闭时执行的回调函数
     */
    lhgdialog.tokenSubmit = function(time,options,callback)
    {
        var icon ='loading.gif';
        options =options||{};
        var reIcon = icon ? function(){
            this.DOM.icon[0].innerHTML = '<img src="' + this.config.path + 'skins/icons/' + icon + '" class="ui_icon_bg"/>';
            this.DOM.icon[0].style.display = '';
        ...
        return lhgdialog({
            ...
            .content(content||'努力请求中，请稍后...')
//            .time(time||5 , reIcon);
            .time(time===undefined?60:time , reIcon);
    };
